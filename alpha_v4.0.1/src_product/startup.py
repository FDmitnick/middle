...
def run():
	...
	quant.initialize()
	# ===设置默认最大杠杆数，变相增加资金容量
	reset_leverage(quant.exchange, max_leverage=3)
	...
	while True:
		...
		# =====获取当前资金费率
		s_time = time.time()
		data, recent = set_fundingrate(exchange)
		premiun_symbol_list = []	# 负费率列表
		for pi in data:
			if float(pi[2]) < 0:
				premiun_symbol_list.append(pi[1])
		print('保存资金数据完成，花费时间：(负费率的如下)', time.time() - s_time, premiun_symbol_list)

		# =====选币数据整理 & 选币
		s_time = time.time()
		select_coin = cal_factor_and_select_coin(
			symbol_candle_data, stratagy_list, run_time, njob2, premiun_symbol_list, min_kline_size=min_kline_size 
		)
		print(select_coin)
