...
def set_fundingrate(exchange, keep_hour=24):
	...
	data, recent = fetch_fundingrate(exchange)
...
def gen_selected(df, select_coin_num, long_weight, short_weight, before_handler, after_handler, premiun_symbol_list):
	df1 = df.copy()
	df2 = df.copy()
	# 前置过滤
	df1, df2 = getattr(filter_handle, before_handler)(df1, df2)
	df2 = df2[~df['symbol'].isin(premiun_symbol_list)].copy()	# 去掉负费率，不做空
...
# =====策略相关函数
# 选币数据整理 & 选币
def cal_factor_and_select_coin(symbol_candle_data, stratagy_list, run_time, njob2, premiun_symbol_list, min_kline_size=999):
	# ===过滤K线
	...
	for strategy in stratagy_list:
		...
		# ===选币
		df = gen_selected(df, select_coin_num, long_weight, short_weight, before_handler, after_handler, premiun_symbol_list)
		# ===处理字段
